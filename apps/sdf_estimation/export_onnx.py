"""
Export a trained UNet3DSDF checkpoint to ONNX.

Usage
-----
# Dynamic spatial dims (default):
    python -m apps.sdf_estimation.export_onnx ^
...
* Install onnxsim (pip install onnxsim) for an optional graph-simplification
  pass after export.
* The exported graph outputs raw float32 SDF values ``[B, num_sdf_fields, D, H, W]``
  -- no activation, no argmax (UNet3DSDF has no Infer/Eval heads).
"""
...
def export(
    ...
    Full export pipeline:
        ...
        2. Reconstruct and populate UNet3DSDF from *checkpoint*.
        3. Apply EMA shadow weights when available.
        4. Trace the raw forward pass and export to *output*.
    ...
    output_names = ["sdf"]
    print("[export] Output: raw SDF predictions [B, num_sdf_fields, D, H, W] (float32)")
    ...
    if not static_shape:
        dynamic_axes = {
            ...
            "sdf": {0: "batch", 2: "depth", 3: "height", 4: "width"},
        }
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Export UNet3DSDF checkpoint to ONNX",
