...
class DataConfig(AbstractConfig):
    ...
    modalities = ConfigField(
        None,   doc='Input modality folder names (None = auto-detect, excluding sdf_names)')
    sdf_names = ConfigField(
        None,   doc='SDF field folder names, one subfolder per field '
    ...
    normalization = ConfigField(
        'znorm', doc='Intensity normalisation applied to modalities only: znorm | rescale | none')
