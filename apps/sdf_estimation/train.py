...
def parse_args() -> argparse.Namespace:
    ...
    g.add_argument('--sdf_names',       nargs='+', required=False, default=None,
    ...
    g.add_argument('--no_elastic',    action='store_true',
...
class Trainer:
    ...
    def _batch_to_tensors(self, batch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        y = torch.cat(
            [batch[s][tio.DATA] for s in self.sdf_names], dim=1
    ...
    def _log_epoch(self, epoch: int, train_m: Dict, val_m: Dict) -> None:
        ...
        for key in ('loss_mse', 'loss_eikonal'):
            if key in train_m:
                self.writer.add_scalar(f'epoch/{key}', train_m[key], epoch)

        self.writer.add_scalars('epoch/mean_mae',
                                {'train': train_m['mean_mae'],
                                 'val':   val_m.get('mean_mae', 0)}, epoch)
        self.writer.add_scalars('epoch/mean_mse',
                                {'train': train_m['mean_mse'],
                                 'val':   val_m.get('mean_mse', 0)}, epoch)
    ...
    def _run_test(self) -> float:
        ...
        for subject in self.test_ds:
            ...
            sdf_gt = torch.cat(
                [subject[s][tio.DATA] for s in self.sdf_names], dim=0
            ...
            json.dumps({
                ...
                'per_subject': [
                    {'subject': getattr(s, 'subject_id', f'sub_{i}'),
                     'mae': float(d), 'mse': float(u)}
                    for i, (s, d, u) in enumerate(zip(self.test_ds, all_mae, all_mse))
                ],
            }, indent=2),
            encoding='utf-8',
        )
