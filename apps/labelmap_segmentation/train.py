...
class Trainer:
    ...
    def _build_scheduler(self):
        ...
        if c.type == "plateau":
            return optim.lr_scheduler.ReduceLROnPlateau(
                o, mode="max", factor=c.factor, patience=c.patience
            )
        if c.type == "step":
    ...
    def _batch_to_tensors(self, batch) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        y = (
            batch[self.data_cfg.label_name][tio.DATA].to(self.device)
            if self.data_cfg.label_name in batch
            else None
        )
        return x, y
    ...
    def train_epoch(self, epoch: int) -> Dict:
        ...
        accum: Dict[str, float] = (
            {}
        )  # all metric sums (loss, dice_*, loss_dice, loss_ce)
            ...
            if i % self.infra_cfg.log_interval == 0:
                ...
                if "dice" in components:
                    self.writer.add_scalar(
                        "train/batch_loss_dice", components["dice"].item(), step
                    )
                if "ce" in components:
                    self.writer.add_scalar(
                        "train/batch_loss_ce", components["ce"].item(), step
                    )
    ...
    def _run_test(self) -> float:
        ...
        for subject in self.test_ds:
            ...
            if self.patch_cfg.enabled:
                ...
                label = (
                    subject[self.data_cfg.label_name][tio.DATA]
                    .unsqueeze(0)
                    .to(self.device)
                )
            else:
        ...
        self.log.info(
            f"Test -- mean Dice: {mean_dice:.4f} ± {std_dice:.4f}  "
            ...
            json.dumps(
                {
                    ...
                    "per_subject": [
                        {
                            "subject": (
                                s.subject_id if hasattr(s, "subject_id") else f"sub_{i}"
                            ),
                            "dice": float(d),
                            "iou": float(u),
                        }
