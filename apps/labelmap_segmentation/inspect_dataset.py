...
class SplitReport:
    ...
    spacing_data: Dict[str, List[tuple]] = field(
        default_factory=dict
    )  # modality -> list
...
class DatasetInspector:
    ...
    def print_report(self, reports: Dict[str, SplitReport]) -> None:
        ...
        for name, rpt in reports.items():
            ...
            print(
                f"  {name:<14} {rpt.n_total:>10} {rpt.n_valid:>8} "
        ...
        for name, rpt in reports.items():
            for mod, spacings in rpt.spacing_data.items():
                ...
                for ax, axis_name in zip(range(arr.shape[1]), ("x", "y", "z")):
                    col = arr[:, ax]
                    print(
                        f"    {axis_name}  mean {col.mean():.3f} +/- {col.std():.3f}"
...
def generate_configs(inspector: DatasetInspector, reports: Dict[str, SplitReport]):
    ...
    if train:
        for mod in inspector.modalities or []:
            ...
            if counter:
                ...
                print(f"  {thin}")
                print(
                    f"    Most common : {most_common_shape}  ({pct:.0f}% of train subjects)"
                )
                if len(counter) > 1:
                    ...
                    print(
                        f"    All subjects share the same shape; no CropOrPad needed."
                    )
            ...
            if spacings:
                ...
                if std.max() > 1e-3:
                    print(
                        f"    Spacing varies (std {std}) -- consider resampling "
    ...
    print(f"  {thin}")
    print(
        f"    flip={ac.flip}  affine={ac.affine}  noise={ac.noise}  "
...
def run_loader_test(
    ...
    try:
        ...
        return
    print(
        f"  train batch_size={train_loader.batch_size}  "
    ...
    try:
        for i, batch in enumerate(train_loader):
            ...
            if lbl_key in batch:
                ...
                print(
                    f"    {lbl_key:<14}  shape={tuple(lbl.shape)}"
                    f"  dtype={lbl.dtype}"
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--label_name", default="label", help="Segmentation mask sub-folder name"
    )
    p.add_argument(
        "--num_classes",
        type=int,
