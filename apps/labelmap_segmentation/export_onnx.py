...
def _simplify(path: str, shape: Tuple[int, ...]) -> None:
    try:
        ...
        if ok:
            ...
            print('[export] onnx-simplifier: simplification not possible -- original kept')
    except ImportError:
        print('[export] onnx-simplifier not installed -- skipping  (pip install onnxsim)')
    except Exception as exc:
...
def export(
    ...
    if train_cfg.ema and 'ema' in ckpt:
        ...
        print('[export] EMA state found in checkpoint but TrainingConfig.ema=False -- skipped')
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help='Path to .pth checkpoint '
             '(e.g. output/exp_001/checkpoints/best.pth)')
    p.add_argument(
        '--output', default='model.onnx',
        help='Output ONNX file path')
    p.add_argument(
        '--head', default='infer', choices=['infer', 'eval'],
    ...
    p.add_argument(
        '--static_shape', action='store_true',
    ...
    p.add_argument(
        '--input_shape', type=int, nargs=3, default=None,
        metavar=('D', 'H', 'W'),
